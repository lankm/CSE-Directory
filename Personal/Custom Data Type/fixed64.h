...
// works for positive numbers and some combinations of negatives
FIX64 mul(FIX64 a, FIX64 b)
{
    ...
    // f*f
    num.f = ((uint64_t)a.f * (uint64_t)b.f) >> 32;
    printf("num: %016llx.%08x\n", num.i, num.f); // print bit values
    ...
    // f*i & i*f
    int64_t temp = (uint64_t)a.f * b.i;
    *(uint64_t*)(&num.f) += temp; // purposly leaking overflow bit into num.i
    printf("num: %016llx.%08x\n", num.i, num.f); // print bit values

    temp = a.i * (uint64_t)b.f;
    *(uint64_t*)(&num.f) += temp; // purposly leaking overflow bit into num.i
    printf("num: %016llx.%08x\n", num.i, num.f); // print bit values
